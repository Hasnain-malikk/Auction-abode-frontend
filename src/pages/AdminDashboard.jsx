...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Ask backend for all properties (include Pending). Backend filters out Pending
                // unless `all` query param is present.
                const propertiesResponse = await fetch('http://localhost:4646/api/properties?all=true');
    ...
    // Normalize and count statuses in a case-insensitive, alias-aware way.
    const pendingProperties = properties.filter(p => ((p.status || '').toString().toLowerCase() === 'pending')).length;

    // Backend sometimes maps 'Approved' -> 'OnSell'. Treat both as approved.
    const approvedProperties = properties.filter(p => {
        const s = (p.status || '').toString().toLowerCase();
        return s === 'approved' || s === 'onsell' || s === 'on sell';
    }).length;

    const rejectedProperties = properties.filter(p => ((p.status || '').toString().toLowerCase() === 'rejected')).length;

    // Keep an explicit On Sell count too (alias aware)
    const onSellProperties = properties.filter(p => {
        const s = (p.status || '').toString().toLowerCase();
        return s === 'onsell' || s === 'on sell';
    }).length;

    // Backend uses 'Selled' for sold; accept 'Selled' or 'Sold'
    const soldProperties = properties.filter(p => {
        const s = (p.status || '').toString().toLowerCase();
        return s === 'selled' || s === 'sold';
    }).length;
